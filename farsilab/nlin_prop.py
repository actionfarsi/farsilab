...
def split_step(A0, z_array,       # Array for solution points
               ...
               self_steep = None, # Self Steepening array
               dt = 1,           # sampling time
               apod = True,      # Boundary conditions
               probe_A0 = None,   # For independent
               probe_beta = None, # Probe may have different beta (e.g. relative group velocity)
               dynamic_predictor = False,
               plot_hook = None, n_plots = 3,  # not used anymore
               tolerance = 0.04, quiet = False):
    ...
    :arg probe_A0: Array with same size than A0, that doesnt undergo self-nonlinear effects.
    :arg probe_w_op: Operator for the Fourier domain for probe (if None, uses w_op)
    
    :arg dynamic_predictor: def False, implement predictor for z_step
    ...
    w = fftfreq(n_points, dt)
    
    probe = probe_A0 is not None
    if probe:
        assert (len(probe_A0) == len(A0))
        probe = True
        Ap_t = probe_A0[:] + 0.j   # Force the array to complex
        Ap_w = fft(Ap_t) * dt
    ...
    def f(delta_z):
        ...
        if probe:
            f.Ap_w = f.Ap_w * f.p_w_exp
            f.Ap_t = ifft(f.Ap_w) / dt
        ...
        if nlin != 0:
            ...
            if self_steep != 0:
                f.dA = ifft(-1j*w*fft(f.A_t))
                f.A_t *= exp(-1 * delta_z * self_steep * (2 * f.dA * f.A_t.conj() + f.A_t * f.dA.conj()))
            ## If there is a probe, it will feel the NL from pump
            ## This is used for interaction with pumps
            if probe:
                f.Ap_t *= exp(-1j * delta_z * nlin * absolute(f.A_t)**2)
    ...
    def initF(delta_z):
            ...
            if probe:
                if probe_w_op is not None:
                    f.p_w_exp = exp(-1j * delta_z/2 * probe_w_op)
                else:
                    f.p_w_exp = exp(-1j * delta_z/2 * w_op)
              
            elif t_op != 0:
                f.t_exp = exp(-1j * delta_z * t_op)
            else:
                f.t_exp = ones_like(A0)
    ...
    fprint("Ready for integration")
    
    ## NLstep < than pi/10
    t_delta_z = 1.*(z_array[1]-z_array[0])/3.1
    nl_delta_z = 0.01*pi/(amax(abs(A_t)**2 * nlin))

    #print (delta_z, nl_delta_z)
    delta_z = min(t_delta_z, nl_delta_z)

    
    ## Loop variable init
    sol_i = 1    
    sols  = [A_t]*len(z_array)
    iters = 0
    loadF()
    initF(delta_z)
    ...
    while z0 < zf:
        ...
        # Force step to land on next desired z step
        if z0 + delta_z > z_array[sol_i]:
             delta_z = (z_array[sol_i] - z0) * 1.1 # To avoid numerical issues
        ...
        ## Cycle check
        #print("Sol {:}, z0 {:.4~} - {:.3~}, dz {:.4~}".format(sol_i,z0,z_array[sol_i],delta_z))
        if z0 >= z_array[sol_i]:
        ...
        if iters % 200 == 0:
            fprint("Iter %8d (to end %8d) %4.2f %%"%(iters,
                        (z_array[-1]-z0)/delta_z,
                        100.*iters/(iters+(z_array[-1]-z0)/delta_z)))
            #print("Sol {:}, z0 {:.4~} - {:.3~}, dz {:.4~}".format(sol_i,z0,z_array[sol_i],delta_z))
                
    fprint("Total iterations: ", iters)
    return sols
...
## Stub
def correlation(a):
    return [ sum(a*roll(a,i))/len(a) for i in range(len(a)) ]
...
def momentum(y, x, n = 2):
    ...
    d0 = (trapz( (x-x0)**n * absolute(y),x))**(1/n)
