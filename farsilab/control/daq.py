...
TaskHandle = uint64 # On 32-bit machines
c_float_p = ctypes.POINTER(float64)
...
class DAQmxError(Exception):
    ...
    def __init__(self, err, desc=None):
      ...
      if type(err) == int:
        self.err  = get_error_string(err).decode('UTF-8')
        self.desc = get_extended_error_info().decode('UTF-8')
      else:
        self.err  = err
        self.desc = desc
...
def get_list_value(object, nidaqfunc):
    return list(map(str.strip, get_string_value(object,nidaqfunc).split(',')))
...
class Device(object):
  def __init__(self, name=b'Dev1'):
    self.name = name.capitalize().encode('UTF-8')
...
class Task(object):
  def __init__(self, device=b'Dev1', task_name=b'', **kwargs):
      self.device = None
  ...
  samples_per_channel = property(fget =
      lambda x: get_value(x.task_handle, nidaq.DAQmxGetSampQuantSampPerChan,
        uint64), fset = lambda x_y: set_value(x_y[0].task_handle,
          nidaq.DAQmxSetSampQuantSampPerChan, uint64(x_y[1])),
  ...
  sample_rate = property(fget =
      ...
      fset = lambda x_y1: set_value(x_y1[0].task_handle, nidaq.DAQmxSetSampClkRate,
        float64(x_y1[1])), doc = 'Number of samples per channel')
...
class InputTask(Task):
  ...
  def read_all_samp(self, just_read_aval = False):
      CHK(nidaq.DAQmxSetReadReadAllAvailSamp(self.task_handle, just_read_aval))

  def add_analog_voltage_channel(self, channel, channel_name=b'',
                                 terminal_config = DAQmx_Val_Cfg_Default,
                                 min_voltage=-10.0,
                                 max_voltage=10.0):
      '''Add an analog voltage channel to an input task.'''
      channel = norm_channel(channel, self.device, b'analog', b'input').encode('UTF-8')
  ...
  def read_analog_float64(self):
      ...
      CHK(nidaq.DAQmxReadAnalogF64(self.task_handle, DAQmx_Val_Cfg_Default,
        float64(10.0), DAQmx_Val_GroupByChannel, data.ctypes.data_as(c_float_p),
        uint32(data.size), ctypes.byref(read), None))
