...
## Parameters
wait = 2      # measurement time
ch  = [2,4]  # channel to acquire  (max 4)
n_aver = 15    # sample to average
...
calls = [ lambda x: rolldata(x,0),
          ...
          lambda x: rolldata(x,3)]

ts = [FreqTask(c,wait) for c in ch]          # Create a task for each channel
dats  = [zeros(n_aver) for c in ch]  # Create the buffer for previus measurement
...
while True:                              # Never stop the measurement
        ...
        print s #, "%.4f"%(average(dats[0])/average(dats[1]))
